...
if (ncol(SNPmat) == length(muts_outflank$Chromosome) && ncol(SNPmat) == length(muts_outflank$Position)) {
  ...
  cat("Length of chromosome vector: ", length(muts_outflank$Chromosome), "\n")
  cat("Length of position vector: ", length(muts_outflank$Position), "\n")
}

# data check - order of locus names = order in SNP matrix and chromosome info

# check if the lengths are the same
if (length(muts_outflank$Affx.ID) != length(colnames(SNPmat))) {
  cat("Mismatch detected: Number of locus names does not match the number of SNP names.\n")
} else {
  # check if the orders are the same
  if (all(muts_outflank$Affx.ID == colnames(SNPmat))) {
    ...
    # identify mismatches
    mismatches <- muts_outflank$Affx.ID != colnames(SNPmat)
    cat("Mismatched names:\n")
    for (i in which(mismatches)) {
      cat("Position:", i, "- Locus Name:", muts_outflank$Affx.ID[i], "- SNP Name:", colnames(SNPmat)[i], "\n")
    }
  }
}

# SNP trimming
newpc <-snp_autoSVD(G=G,infos.chr =muts_outflank$Chromosome,infos.pos = muts_outflank$Position)
which_pruned <- attr(newpc, which="subset") # indexes of remaining SNPS after pruning

length(which_pruned) # 104957 - same as thinned snp set - this makes sense I think
