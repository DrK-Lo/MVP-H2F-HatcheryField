...
# check dimensions
dim(gen) # thinned - 154 individuals and 104958 SNPs
dim(muts) # full mutation-level info - 154609 SNPs
...
# now filter the mutation df again to just retain SNPs that are not all heterozygous
...
dim(fst_data) # 104959 - sample id col = 104958
...
ggsave(file.path("..", "..", "figures", "pairwise_differences_analyses", "fig_fst_heatmap.png"),  fig_fst_heatmap, width = 8, height = 6)
...
colnames(full)[col_heterozygous] #Affx-1246226248 - same as before
...
# rename for processing
basic_stats_data <- FULL_filtered_fst

# make sample IDs into a real column (instead of rownames)
sample_IDs <- rownames(basic_stats_data)
basic_stats_data$merge_ID_inds <- sample_IDs # "merge_ID_inds" is the column name in samp_full_subset

# reset the rownames to empty
rownames(basic_stats_data) <- NULL

# make sure labels in fst data still line up with sample data
if(!identical((basic_stats_data$merge_ID_inds),samp_full_subset$merge_ID_inds)){
    print("Error sample names don't line up")
    break
   } # passes

# store data to check labels 
fst_labels_check <- basic_stats_data$merge_ID_inds
head(fst_labels_check)
samp_labels_check <- samp_full_subset$merge_ID_inds
head(samp_labels_check)
...
# add population labels to full snp data
...
# sanity check: number of columns 
dim(basic_stats_data) # number of SNPs (154608) + sample col + pop col = 154610
...
# rename for processing - full SNP set already cleaned
...
a = sprintf("%08d", muts_full_filtered$Position)
muts_full_filtered$chrom_position <- paste(muts_full_filtered$Chromosome, a, sep = ".")

# add chromosome positions to dataframe
P1$Affx.ID <- P1$LocusName
joined_P1 <- left_join(P1, muts_full_filtered, by = c("Affx.ID" = "Affx.ID"))
