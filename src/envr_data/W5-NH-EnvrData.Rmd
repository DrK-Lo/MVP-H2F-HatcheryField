...
raw_W5_NH <- read_csv("data/environment/W5-NH/W5-NH-raw.csv")
...
raw_W5_NH$datetime <- as.POSIXct(raw_W5_NH$DateTimeStamp, format = "%m/%d/%y %H:%M", tz = "UTC")
...
print(summary(W5_NH$salinity))
print(summary(W5_NH$temp))
```
...
# add site name
W5_NH <- W5_NH %>% 
  mutate(site_name = "W5-NH") %>%
  select(site_name, everything())

# create temp subset: site_name, datetime, temp
W5_NH_temp_full <- W5_NH %>% 
  select(site_name, datetime, temp)

# create salinity subset: site_name, datetime, salinity
W5_NH_sal_full <- W5_NH %>% 
  select(site_name, datetime, salinity)

# save as csv for future analyses
write.csv(W5_NH_temp_full, "data/environment/W5-NH/W5-NH_temp_full.csv", row.names = FALSE)
write.csv(W5_NH_sal_full, "data/environment/W5-NH/W5-NH_sal_full.csv", row.names = FALSE)
```
...
### Low salinity stress
```{r low-sal-stress}
# calculate stress inline so we don't add extra columns to W5_NH
...
ggplot(low_sal_stress_count, aes(x = factor(month), y = low_sal_stress, fill = factor(month))) +
  ...
  facet_wrap(~ year) +
  labs(title = "W5-NH: count of low salinity (< 12 ppt) days per month",
       x = "month",
       y = "count of days") +
  theme_minimal()
...
ggplot(high_temp_stress_count, aes(x = factor(month), y = high_temp_stress, fill = factor(month))) +
  ...
  facet_wrap(~ year) +
  labs(title = "W5-NH: count of high temperature (> 28 C) days per month",
       x = "month",
       y = "count of days") +
  theme_minimal()
...
temp_quantile_10 <- unname(quantile(W5_NH$temp, 0.1))
temp_quantile_90 <- unname(quantile(W5_NH$temp, 0.9))
...
W5_NH_monthly_temp <- Mean_Monthly_Temperature_C %>%
...
# write.table(W5_NH_temp, "data/environment/all_temperature.csv", sep = ",", append = TRUE, col.names = FALSE, row.names = FALSE)

# write to a unique new CSV file
write.csv(W5_NH_temp, "data/environment/W5-NH/W5-NH_temperature.csv", row.names = FALSE)

# write all monthly data to a unique new CSV file
write.csv(W5_NH_monthly_temp, "data/environment/W5-NH/W5-NH_monthly_temp.csv", row.names = FALSE)
```
...
salinity_quantile_10 <- unname(quantile(W5_NH$salinity, 0.1))
salinity_quantile_90 <- unname(quantile(W5_NH$salinity, 0.9))
...
W5_NH_monthly_sal <- Mean_Monthly_Salinity %>%
  left_join(Min_Monthly_Salinity, by = "month") %>%
  left_join(Max_Monthly_Salinity, by = "month")
print(W5_NH_monthly_sal)

# write to the combined file with all sites 
# write.table(W5_NH_salinity, "data/environment/all_salinity.csv", sep = ",", append = TRUE, col.names = FALSE, row.names = FALSE)

# write all year data to a unique new CSV file
write.csv(W5_NH_salinity, "data/environment/W5-NH/W5-NH_salinity.csv", row.names = FALSE)
# write all monthly data to a unique new CSV file
write.csv(W5_NH_monthly_sal, "data/environment/W5-NH/W5-NH_monthly_sal.csv", row.names = FALSE)
```