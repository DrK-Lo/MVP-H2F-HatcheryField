...
raw_W1_TX <- read_csv("data/environment/W1-TX/W1-TX-raw.csv")
...
raw_W1_TX$datetime <- as.POSIXct(raw_W1_TX$DateTimeStamp, format = "%m/%d/%y %H:%M", tz = "UTC")
...
print(summary(W1_TX$salinity))
print(summary(W1_TX$temp))
```
...
# add site name
W1_TX <- W1_TX %>% 
  mutate(site_name = "W1-TX") %>%
  select(site_name, everything())

# create temp subset: site_name, datetime, temp
W1_TX_temp_full <- W1_TX %>% 
  select(site_name, datetime, temp)

# create salinity subset: site_name, datetime, salinity
W1_TX_sal_full <- W1_TX %>% 
  select(site_name, datetime, salinity)

# save as csv for future analyses
write.csv(W1_TX_temp_full, "data/environment/W1-TX/W1-TX_temp_full.csv", row.names = FALSE)
write.csv(W1_TX_sal_full, "data/environment/W1-TX/W1-TX_sal_full.csv", row.names = FALSE)
```
...
### Low salinity stress
```{r low-sal-stress}
# calculate stress inline so we don't add extra columns to W1_TX
...
ggplot(low_sal_stress_count, aes(x = factor(month), y = low_sal_stress, fill = factor(month))) +
  ...
  facet_wrap(~ year) +
  labs(title = "W1-TX: count of low salinity (< 12 ppt) days per month",
       x = "month",
       y = "count of days") +
  theme_minimal()
...
ggplot(high_sal_stress_count, aes(x = factor(month), y = high_sal_stress, fill = factor(month))) +
  ...
  facet_wrap(~ year) +
  labs(title = "W1-TX: count of high salinity (> 35 ppt) days per month",
       x = "month",
       y = "count of days") +
  theme_minimal()
...
ggplot() +
  ...
  facet_wrap(~ year) +
  labs(title = "W1-TX: count of stressful salinity days per month", 
       x = "month", 
       y = "count of days") + 
  theme_minimal()
...
ggplot(high_temp_stress_count, aes(x = factor(month), y = high_temp_stress, fill = factor(month))) +
  ...
  facet_wrap(~ year) +
  labs(title = "W1-TX: count of high temperature (> 28 C) days per month",
       x = "month",
       y = "count of days") +
  theme_minimal()
...
temp_quantile_10 <- unname(quantile(W1_TX$temp, 0.1))
temp_quantile_90 <- unname(quantile(W1_TX$temp, 0.9))
...
W1_TX_monthly_temp <- Mean_Monthly_Temperature_C %>%
...
# write.table(W1_TX_temp, "data/environment/all_temperature.csv", sep = ",", append = TRUE, col.names = FALSE, row.names = FALSE)

# write to a unique new CSV file
write.csv(W1_TX_temp, "data/environment/W1-TX/W1-TX_temperature.csv", row.names = FALSE)

# write all monthly data to a unique new CSV file
write.csv(W1_TX_monthly_temp, "data/environment/W1-TX/W1-TX_monthly_temp.csv", row.names = FALSE)
```
...
salinity_quantile_10 <- unname(quantile(W1_TX$salinity, 0.1))
salinity_quantile_90 <- unname(quantile(W1_TX$salinity, 0.9))
...
W1_TX_monthly_sal <- Mean_Monthly_Salinity %>%
  left_join(Min_Monthly_Salinity, by = "month") %>%
  left_join(Max_Monthly_Salinity, by = "month")
print(W1_TX_monthly_sal)

# write to the combined file with all sites 
# write.table(W1_TX_salinity, "data/environment/all_salinity.csv", sep = ",", append = TRUE, col.names = FALSE, row.names = FALSE)

# write all year data to a unique new CSV file
write.csv(W1_TX_salinity, "data/environment/W1-TX/W1-TX_salinity.csv", row.names = FALSE)
# write all monthly data to a unique new CSV file
write.csv(W1_TX_monthly_sal, "data/environment/W1-TX/W1-TX_monthly_sal.csv", row.names = FALSE)
```