...
```{r}
#Data was downloaded on 10/14/2025
#Source - Dr. William "Willy" Reay of VIMS shared the file with Jess Small via email. Jess emailed the file to KEL who shared it with CR & NM.
#The site was sampled continuously every 15 min from May 2023 to April 2024

#Create text strings with metadata information that we want to include in the final data frame. 
download_date <- ("10-14-2025")
...
setwd("/Users/camillerumberger/Documents/GitHub/MVP-H2F-HatcheryField")

#files
raw_LOLA_env21 <- read.csv("data/environment/Jess_COAN_2021.csv")
raw_LOLA_env22 <- read.csv("data/environment/Jess_COAN_2022.csv")
raw_LOLA_env25 <- read.csv("data/environment/Jess_COAN_2025.csv")
raw_LOLA_env23_24 <- read.csv("data/environment/LEW-raw.csv")

head(raw_LOLA_env23_24)
...
write.csv(LOLA_temp_full, "data/envr_raw_data/full_datasets/full_temp/LOLA_temp_full.csv", row.names = FALSE)

write.csv(LOLA_sal_full, "data/envr_raw_data/full_datasets/full_sal/LOLA_sal_full.csv", row.names = FALSE)
```
...
summary(count.nas_LOLA_env) # no nas

nrow(LOLA_env) # figure out how many rows we have in the original df: 153925
LOLA_env <- na.omit(LOLA_env) #remove NAs using na.omit
nrow(LOLA_env) # none removed
...
# there is a time skip between 2022-12-31 23:45:00 - 2023-05-01 00:00:00
```
...
LOLA_temp <- cbind(site_name, download_date, source_description, lat, lon, firstyear, finalyear, Mean_Annual_Temperature_C, Mean_max_temperature_C, Mean_min_temperature_C, Temperature_st_dev, Temperature_n, Temperature_years)
print(LOLA_temp)

# Write to a unique new CSV file
write.csv(LOLA_temp, "data/environment/LOLA_temperature.csv", row.names = FALSE)
...
#Create a data frame to store the temperature results
LOLA_salinity <- cbind(site_name, download_date, source_description, lat, lon, firstyear, finalyear, Mean_Annual_Salinity_ppt, Mean_max_Salinity_ppt, Mean_min_Salinity_ppt, Salinity_st_dev, Salinity_n, Salinity_years)
print(LOLA_salinity)

# Write to the combined file with all sites 
# Write to a unique new CSV file
write.csv(LOLA_temp, "data/environment/LOLA_salinity.csv", row.names = FALSE)
```