...
```{r}
# the file we will be working with is from Timucuan Preserve, Florida. The ID_Site for this site is W3_FL. 
raw_W3_FL_sal <- read_csv("data/environment/W3-FL/W3-FL-raw_sal.csv")
raw_W3_FL_temp <- read_csv("data/environment/W3-FL/W3-FL-raw_temp.csv")
...
raw_W3_FL_sal$datetime <- as.POSIXct(raw_W3_FL_sal$Timestamp, format = "%m/%d/%y %H:%M", tz = "UTC")
raw_W3_FL_temp$datetime <- as.POSIXct(raw_W3_FL_temp$Timestamp, format = "%m/%d/%y %H:%M", tz = "UTC")
...
```{r}
# filter the data between the values of 0 and 42 for salinity and 0 and 40 for temperature. 
W3_FL_sal <- raw_W3_FL_sal %>%
    filter(between(salinity, 0, 42)) 
           
W3_FL_temp <- raw_W3_FL_temp %>%
    filter(between(temp, 0, 40))

# sanity check - print the ranges to ensure values are filtered properly. We can see that the ranges for both are now in the appropriate range.  
print(summary(W3_FL_sal$salinity))
print(summary(W3_FL_temp$temp))
```

## Datasets for violin plots
```{r}
# create temp subset: site_name, datetime, temp
W3_FL_temp_full <- W3_FL_temp %>% 
  mutate(site_name = "W3-FL") %>%
  select(site_name, datetime, temp)

# create salinity subset: site_name, datetime, salinity
W3_FL_sal_full <- W3_FL_sal %>% 
  mutate(site_name = "W3-FL") %>%
  select(site_name, datetime, salinity)

# save as csv for future analyses
write.csv(W3_FL_temp_full, "data/environment/W3-FL/W3-FL_temp_full.csv", row.names = FALSE)
write.csv(W3_FL_sal_full, "data/environment/W3-FL/W3-FL_sal_full.csv", row.names = FALSE)
```
...
### Low salinity stress 
```{r low-sal-stress}
# calculate stress inline so we don't add extra columns to W3_FL_sal
...
ggplot(low_sal_stress_count, aes(x = factor(month), y = low_sal_stress, fill = factor(month))) +
  ...
  facet_wrap(~ year) +
  labs(title = "W3-FL: count of low salinity (< 12 ppt) days per month",
       x = "month",
       y = "count of days") +
  theme_minimal()
...
ggplot(high_sal_stress_count, aes(x = factor(month), y = high_sal_stress, fill = factor(month))) +
  ...
  facet_wrap(~ year) +
  labs(title = "W3-FL: count of high salinity (> 35 ppt) days per month",
       x = "month",
       y = "count of days") +
  theme_minimal()
...
ggplot() +
  ...
  facet_wrap(~ year) +
  labs(title = "W3-FL: count of stressful salinity days per month", 
       x = "month", 
       y = "count of days") + 
  theme_minimal()
...
ggplot(high_temp_stress_count, aes(x = factor(month), y = high_temp_stress, fill = factor(month))) +
  ...
  facet_wrap(~ year) +
  labs(title = "W3-FL: count of high temperature (> 28 C) days per month",
       x = "month",
       y = "count of days") +
  theme_minimal()
...
temp_quantile_10 <- unname(quantile(W3_FL_temp$temp, 0.1))
temp_quantile_90 <- unname(quantile(W3_FL_temp$temp, 0.9))
...
# NOTE: use W3_FL_temp_summary to avoid overwriting the W3_FL_temp data frame
...
print(W3_FL_temp_summary)

# join monthly temp data instead of cbind + dedup
W3_FL_monthly_temp <- Mean_Monthly_Temperature_C %>%
...
# write.table(W3_FL_temp_summary, "data/environment/all_temperature.csv", sep = ",", append = TRUE, col.names = FALSE, row.names = FALSE)

# write to a unique new CSV file
write.csv(W3_FL_temp_summary, "data/environment/W3-FL/W3-FL_temperature.csv", row.names = FALSE)

# write all monthly data to a unique new CSV file
write.csv(W3_FL_monthly_temp, "data/environment/W3-FL/W3-FL_monthly_temp.csv", row.names = FALSE)
```
...
salinity_quantile_10 <- unname(quantile(W3_FL_sal$salinity, 0.1))
salinity_quantile_90 <- unname(quantile(W3_FL_sal$salinity, 0.9))
...
W3_FL_monthly_sal <- Mean_Monthly_Salinity %>%
  left_join(Min_Monthly_Salinity, by = "month") %>%
  left_join(Max_Monthly_Salinity, by = "month")
print(W3_FL_monthly_sal)

# write to the combined file with all sites 
# write.table(W3_FL_salinity, "data/environment/all_salinity.csv", sep = ",", append = TRUE, col.names = FALSE, row.names = FALSE)

# write all year data to a unique new CSV file
write.csv(W3_FL_salinity, "data/environment/W3-FL/W3-FL_salinity.csv", row.names = FALSE)
# write all monthly data to a unique new CSV file
write.csv(W3_FL_monthly_sal, "data/environment/W3-FL/W3-FL_monthly_sal.csv", row.names = FALSE)
```