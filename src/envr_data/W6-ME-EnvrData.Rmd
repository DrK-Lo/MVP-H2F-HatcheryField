...
raw_W6_ME <- read_csv("data/environment/W6-ME/W6-ME-raw.csv")
...
raw_W6_ME$datetime <- as.POSIXct(raw_W6_ME$Timestamp, format = "%m/%d/%y %H:%M", tz = "UTC")
...
print(summary(W6_ME$salinity))
print(summary(W6_ME$temp))

# check if we have NAs
count.nas_datetime <- is.na(W6_ME$datetime)
...
# add site name
W6_ME <- W6_ME %>% 
  mutate(site_name = "W6-ME") %>%
  select(site_name, everything())

# create temp subset: site_name, datetime, temp
W6_ME_temp_full <- W6_ME %>% 
  select(site_name, datetime, temp)

# create salinity subset: site_name, datetime, salinity
W6_ME_sal_full <- W6_ME %>% 
  select(site_name, datetime, salinity)

# save as csv for future analyses
write.csv(W6_ME_temp_full, "data/environment/W6-ME/W6-ME_temp_full.csv", row.names = FALSE)
write.csv(W6_ME_sal_full, "data/environment/W6-ME/W6-ME_sal_full.csv", row.names = FALSE)
```
...
```{r timeplot}
timeplot <- ggplot(W6_ME_envrmonth_sal, aes(x = year)) +
    geom_point(aes(y = month, color = length_salinity), size = 4) +
    labs(x = "Time", y = "Month", title = "Timeplot for W6_ME") +
    ylim(1, 12) +
    theme_minimal()
...
### Low salinity stress
```{r low-sal-stress}
# calculate stress inline so we don't add extra columns to W6_ME
...
ggplot(low_sal_stress_count, aes(x = factor(month), y = low_sal_stress, fill = factor(month))) +
  ...
  facet_wrap(~ year) +
  labs(title = "W6-ME: count of low salinity (< 12 ppt) days per month",
       x = "month",
       y = "count of days") +
  theme_minimal()
...
temp_quantile_10 <- unname(quantile(W6_ME$temp, 0.1))
temp_quantile_90 <- unname(quantile(W6_ME$temp, 0.9))
...
W6_ME_monthly_temp <- Mean_Monthly_Temperature_C %>%
...
#write.table(W6_ME_temp, "data/environment/all_temperature.csv", sep = ",", append = TRUE, col.names = FALSE, row.names = FALSE)

# write to a unique new CSV file
write.csv(W6_ME_temp, "data/environment/W6-ME/W6-ME_temperature.csv", row.names = FALSE)

# write all monthly data to a unique new CSV file
write.csv(W6_ME_monthly_temp, "data/environment/W6-ME/W6-ME_monthly_temp.csv", row.names = FALSE)
```
...
salinity_quantile_10 <- unname(quantile(W6_ME$salinity, 0.1))
salinity_quantile_90 <- unname(quantile(W6_ME$salinity, 0.9))
...
W6_ME_monthly_sal <- Mean_Monthly_Salinity %>%
  left_join(Min_Monthly_Salinity, by = "month") %>%
  left_join(Max_Monthly_Salinity, by = "month")
print(W6_ME_monthly_sal)

# write to the combined file with all sites 
# write.table(W6_ME_salinity, "data/environment/all_salinity.csv", sep = ",", append = TRUE, col.names = FALSE, row.names = FALSE)

# write all year data to a unique new CSV file
write.csv(W6_ME_salinity, "data/environment/W6-ME/W6-ME_salinity.csv", row.names = FALSE)
# write all monthly data to a unique new CSV file
write.csv(W6_ME_monthly_sal, "data/environment/W6-ME/W6-ME_monthly_sal.csv", row.names = FALSE)
```