...
packages <- c("purrr", "here", "ggplot2", "tidyverse", "grid", "reshape2", "vegan", "patchwork")
...
clean_site_names <- function(df, site_col = "site_name") {
  old_names <- names(SITE_RECODE)
  new_names <- unname(SITE_RECODE)
  df %>%
    filter(!.data[[site_col]] %in% EXCLUDE_GROUPS) %>%
    mutate(!!site_col := case_match(
      .data[[site_col]],
      old_names ~ new_names,
      .default = .data[[site_col]]
    ))
}
...
split_field_data <- function(df, prefix, group_col = "bag_site") {
  nested <- df %>%
    group_by(monitoring_event, .data[[group_col]]) %>%
    nest() %>%
    mutate(
      name = paste0(prefix, "_event", monitoring_event, "_",
                    tolower(gsub(" ", "", .data[[group_col]]))),
      data = map(data, ~ .x %>% order_by_pop() %>% dplyr::select(site_name, value))
    )

  setNames(nested$data, nested$name)
}
...
# matrix of euclidean environmental distances using SCALED data for environment-of-origin sites only
envr_dist <- readRDS(file.path("results", "environment", "envr_quantiles_dist_origin.rds"))
...
surv_day21 <- hatchery_survival %>%
  dplyr::filter(Day_analysis == "15-21", !grepl("small", Group_Day_Label, ignore.case = TRUE)) %>%
...
response_heatmaps <- list(
  ...
  Fig4C_survival_lew = plot_heatmap(dist_matrices$surv_event3_lewisetta, "Euclidean distance\nin survival"),
  Fig4D_survival_york = plot_heatmap(dist_matrices$surv_event3_yorkriver, "Euclidean distance\nin survival")
)
