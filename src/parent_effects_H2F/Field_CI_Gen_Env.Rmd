...
env_parents <- read.csv("../../data/environment/quantiles_05232025.csv")[,-c(1, 9)]
...
#plot(ci_t2_lew) 
Anova(ci_t2_lew, type = "III") # temp10 signif
summary(ci_t2_lew)
step(ci_t2_lew) # drop ho, ar, sal quant10

ci_t2_lew2 <- lmerTest::lmer(ci ~ temp_quantile_10 + salinity_quantile_90 + (1|bags_label), data = ci_gen_env[ci_gen_env$t == "t2" & ci_gen_env$bag_site == "Lewisetta",]) 
#plot(ci_t2_lew2) 
Anova(ci_t2_lew2, type = "III") # temp10, sal90 signif
summary(ci_t2_lew2)
step(ci_t2_lew2) #says to drop random effect, we will keep it for consistency among all models

ci_t2_lew_scaled <- lmerTest::lmer(scale(ci) ~ scale(temp_quantile_10) + scale(salinity_quantile_90) + (1|bags_label), data = ci_gen_env[ci_gen_env$t == "t2" & ci_gen_env$bag_site == "Lewisetta",])
```
...
summary(ci_t3_lew)
step(ci_t3_lew) # drop everything but sal10, we will keep the random effect for consistency among all models

ci_t3_lew2 <- lmerTest::lmer(ci ~ salinity_quantile_10 + (1|bags_label), data = ci_gen_env[ci_gen_env$t == "t3" & ci_gen_env$bag_site == "Lewisetta",])

#plot(ci_t3_lew) 
Anova(ci_t3_lew2) #sal10 signif
summary(ci_t3_lew2)

ci_t3_lew_scaled <- lmerTest::lmer(scale(ci) ~ scale(salinity_quantile_10) + (1|bags_label), data = ci_gen_env[ci_gen_env$t == "t3" & ci_gen_env$bag_site == "Lewisetta",])
```
...
#plot(ci_t1_yrk) 
Anova(ci_t1_yrk, type = "III") # sal10 signif
summary(ci_t1_yrk)
...
summary(ci_t2_yrk)
step(ci_t2_yrk) # drop everything but ar and temp10, keep random effect

ci_t2_yrk2 <-lmerTest::lmer(ci ~ ar + temp_quantile_10 + (1|bags_label), data = ci_gen_env[ci_gen_env$t == "t2" & ci_gen_env$bag_site == "YorkRiver",]) 
#plot(ci_t2_yrk2) 
Anova(ci_t2_yrk2, type = "III") #both signif
summary(ci_t2_yrk2)
step(ci_t2_yrk2) # drop random effect, ignoring this for reasons mentioned above

ci_t2_yrk_scaled <- lmerTest::lmer(scale(ci) ~ scale(ar) + scale(temp_quantile_10) + (1|bags_label), data = ci_gen_env[ci_gen_env$t == "t2" & ci_gen_env$bag_site == "YorkRiver",])
```
...
#plot(dist_t3_lew) 
Anova(dist_t3_lew, type = "III") # ho, ar signif
summary(dist_t3_lew)
step(dist_t3_lew) # drop random effect

dist_t3_lew2 <-lm(ci ~ ho + ar + dist_LEW, data = ci_gen_dist[ci_gen_dist$t == "t3" & ci_gen_dist$bag_site == "Lewisetta",]) 
#plot(dist_t3_lew2) 
summary(dist_t3_lew2) #all signif
drop1(dist_t3_lew2) #final model

#use lmer with random effect for plotting, but note that excluding the random effect makes envr distance a significant predictor (but barely, p = 0.034)
...
#t2
lew_ci1_df_sub[nrow(lew_ci1_df_sub)+1,]=list("scale(ho)",-.4,.01,NA,NA,NA,"t2")
...
lew_ci1_df_sub[nrow(lew_ci1_df_sub)+1,]=list("scale(ar)",-.4,.01,NA,NA,NA,"t3")
...
lew_ci1_plot <- lew_ci1_df_sub %>% 
  ...
  scale_alpha_manual(name = "Monitoring event", na.translate = F, values = c("t1" = .45, "t2" = .7, "t3" = 1), labels = c("t1" = "Nov. 2023", "t2" = "May 2024", "t3" = "Nov. 2024"))+
  scale_shape_manual(name = "Monitoring event", na.translate = F, values = c("t1" = 15, "t2" = 16, "t3" = 17), labels = c("t1" = "Nov. 2023", "t2" = "May 2024", "t3" = "Nov. 2024"))+
  xlim(c(-1,2))+
...
yrk_ci2_plot <- yrk_ci2_df_sub %>% 
  ...
  geom_point(aes(alpha = t, shape = t), color = "#880E4F", size = 3.5, position = position_dodge2v(height = .85, reverse = T, preserve = "total"))+
  geom_errorbarh(aes(xmax = slope + 2*se, xmin = slope - 2*se, alpha= t), color = "#880E4F", height = 0, size = 1.2, position = position_dodgev(height = -.85, preserve = "total"))+
  scale_alpha_manual(name = "Monitoring event", na.translate = F, 
                     values = c("t1" = .45, "t2" = .7, "t3" = 1), 
                     labels = c("t1" = "Nov. 2023", "t2" = "May 2024", "t3" = "Nov. 2024"))+
  scale_shape_manual(name = "Monitoring event", na.translate = F, 
