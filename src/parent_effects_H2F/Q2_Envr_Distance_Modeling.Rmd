---
title: "Q2_Envr_Distance_Len_Surv"
...
pdf(file = "../../figures/parent_effects_H2F/Lew_EnvrDist_Genetics_Corrplot.pdf")
corrplot_lew <- corrplot(cor_matrix_lew,
         method = "color",
         type = "upper",
         tl.col = "black",
         tl.srt = 45,
         title = "Lewisetta: Genetics Variables and Envr. Distance Multicollinearity",
         addCoef.col = 'black',
         mar=c(0,0,2,0))
dev.off()
...
colnames(cor_matrix_yrk) <- c("Distance to York","Heterozygosity","Allelic Richness")

rownames(cor_matrix_yrk) <- c("Distance to York","Heterozygosity","Allelic Richness")

pdf(file = "../../figures/parent_effects_H2F/Yrk-EnvrDist_Genetics_Corrplot.pdf")
corrplot_yrk <- corrplot(cor_matrix_yrk,
         method = "color",
         type = "upper",
         tl.col = "black",
         tl.srt = 45,
         title = "York River: Genetics Variables and Envr. Distance Multicollinearity",
         addCoef.col = 'black',
         mar=c(0,0,2,0))
dev.off()
#nothing collinear
...
Now look at how survival changes with genetics/environment at each time point. Modeling chunks below are based on those by CR with variables switched out as needed.
...
step(len_yrk_t2.0) # drop everything except bags label??

#try cutting dist_YRK, least significant among the variables
len_yrk_t2.1 <- lmerTest::lmer(length ~ ho + ar + (1|bags_label), data = len_mod_gen_dist[len_mod_gen_dist$t == 2 & len_mod_gen_dist$bag_site == "YorkRiver",]) 
Anova(len_yrk_t2.1, type = "III") #only ar signif
summary(len_yrk_t2.1)
step(len_yrk_t2.1) #still recommends cutting everything except bag label

len_yrk_t2.2 <- lmerTest::lmer(length ~ (1|bags_label), data = len_mod_gen_dist[len_mod_gen_dist$t == 2 & len_mod_gen_dist$bag_site == "YorkRiver",]) 
step(len_yrk_t2.2) #final model
...
#add column for sampling time
lew_len_dist_df$t <- c(rep(c("t1"), 4), rep(c("t2"), 5), rep(c("t3"), 5))
...
#add dummy rows for time points lacking variables so that position_dodge v plots each data point in the same time point at consistent vertical positions for each y axis variable
lew_len_dist_df2[nrow(lew_len_dist_df2) +1, ] = list("scale(ar)", -.5, .01, NA, NA, "t1")
lew_len_dist_df2[nrow(lew_len_dist_df2) +1, ] = list("scale(dist_LEW)", -.5, .01, NA, NA, "t1")
...
yrk_len_dist_df <- rbind(broom.mixed::tidy(len_t3_yrk_s))

#add column for sampling time
yrk_len_dist_df$t <- c("t3")
#rename columns
...
#add dummy rows for time points lacking variables so that position_dodge v plots each data point in the same time point at consistent vertical positions for each y axis variable
yrk_len_dist_df2[nrow(yrk_len_dist_df2) +1, ] = list("scale(ar)", -.5, .01, NA, NA, "t1")
yrk_len_dist_df2[nrow(yrk_len_dist_df2) +1, ] = list("scale(ho)", -.5, .01, NA, NA, "t1")
yrk_len_dist_df2[nrow(yrk_len_dist_df2) +1, ] = list("scale(dist_YRK)", -.5, .01, NA, NA, "t1")
yrk_len_dist_df2[nrow(yrk_len_dist_df2) +1, ] = list("scale(ar)", -.5, .01, NA, NA, "t2")
yrk_len_dist_df2[nrow(yrk_len_dist_df2) +1, ] = list("scale(ho)", -.5, .01, NA, NA, "t2")
yrk_len_dist_df2[nrow(yrk_len_dist_df2) +1, ] = list("scale(dist_YRK)", -.5, .01, NA, NA, "t2")
