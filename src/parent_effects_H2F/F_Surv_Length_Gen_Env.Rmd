...
This script is for modeling differences in survival and length amongst bags/populations/sites/time points in the MVP Field Experiment. Modeling code is based on code structure written by Camille Rumberger with edits by NM for specific variables used in these analyses.

CR: "I intend to use linear (mixed) models to examine how survival and length vary by group and how genetics and environmental history drive these differences in performance."
...
pdf(file = "../../figures/parent_effects_H2F/Envr_Genetics_Corrplot.pdf")
corrplot_quant <- corrplot(cor_matrix_quant,
         method = "color",
         type = "upper",
         tl.col = "black",
         tl.srt = 45,
         title = "Environmental Quantiles and Genetics Variables Collinearity",
         addCoef.col = 'black',
         mar=c(0,0,2,0))
dev.off()
...
#plot(surv_GE2.lew)
summary(surv_GE2.lew) # ho, temp_quantile_10 signif
...
#plot(surv_GE2.0.lew)
summary(surv_GE2.0.lew) # intercept, ho, ar
...
#plot(surv_GE2.york)
summary(surv_GE2.york) # ar, salinity_quantile_10, temp_quantile_10 signif
...
#plot(surv_GE3.york)
summary(surv_GE3.york) # intercept, ar, salinity_quantile_10 signif
...
# save everything
sink(paste0("../../results_field/survival_GE_model1_results_",Sys.Date(),".txt"))
cat("\nLEW t1 modeling approach 1\n")
summary(surv_GE1.1.lew)
cat("\nLEW t1 scaled modeling approach 1\n")
summary(surv_t1_lew_scaled)
cat("\nLEW t2 modeling approach 1\n")
summary(surv_GE2.0.lew)
cat("\nLEW t2 scaled modeling approach 1\n")
summary(surv_t2_lew_scaled)
cat("\nLEW t3 modeling approach 1\n")
summary(surv_GE3.lew)
cat("\nLEW t3 scaled modeling approach 1\n")
summary(surv_t3_lew_scaled)
cat("\nYRK t1 modeling approach 1\n")
summary(surv_GE1.0.york)
cat("\nYRK t1 scaled modeling approach 1\n")
summary(surv_t1_yrk_scaled)
cat("\nYRK t2 modeling approach 1\n")
summary(surv_GE2.0.york)
cat("\nYRK t2 scaled modeling approach 1\n")
summary(surv_t2_yrk_scaled)
cat("\nYRK t3 modeling approach 1\n")
summary(surv_GE3.0.york)
cat("\nYRK t3 scaled modeling approach 1\n")
...
#plot(len_GE2.york)
Anova(len_GE2.york, type = "III") # all except salinity_quantile_90 and temp quant 10 signif
summary(len_GE2.york)
step(len_GE2.york) # drop salinity_quantile_90 and temp10

len_GE2.0.york <- lmer(length ~ ho + ar + salinity_quantile_10 + (1|bags_label), data = len_mod_gen_env[len_mod_gen_env$t == 2 & len_mod_gen_env$bag_site == "YorkRiver",])
len_GE2.0.york
#plot(len_GE2.0.york)
...
#make scaled version for plotting
len_t2_yrk_scaled <- lmer(scale(length) ~ scale(ho) + scale(ar) + scale(salinity_quantile_10) + (1|bags_label), data = len_mod_gen_env[len_mod_gen_env$t == 2 & len_mod_gen_env$bag_site == "YorkRiver",])
len_t2_yrk_scaled
```
...
sink(paste0("../../results_field/length_GE_model1_results_",Sys.Date(),".txt"))
...
yrk_surv_plot <- yrk_surv_df %>% 
  ...
  scale_alpha_manual(name = "Time", na.translate = F, values = c("t1" = .4, "t2" = .7, "t3" = 1), labels = c("t1" = "Nov. 2023", "t2" = "May 2024", "t3" = "Nov. 2024"))+
  scale_shape_manual(name = "Time", na.translate = F, values = c("t1" = 15, "t2" = 16, "t3" = 17), labels = c("t1" = "Nov. 2023", "t2" = "May 2024", "t3" = "Nov. 2024"))+
  xlim(c(-1,2))+
  xlab("Slope on Scaled Data")+
  ylab("Variable")+
...
yrk_len_df$t <- c(rep(c("t1"), 6), rep(c("t2"), 6), rep(c("t3"), 7))
...
yrk_len_df2[nrow(yrk_len_df2) + 1,] = list("scale(temp_quantile_10)", -.4, .01, NA, NA, NA, "t2")
yrk_len_df2[nrow(yrk_len_df2) + 1,] = list("scale(salinity_quantile_90)", -.4, .01, NA, NA, NA, "t2")
...
surv_mod_gen_env %>% 
  ...
  ggplot(aes(x = salinity_quantile_10, y = surv))+
  geom_point()+
