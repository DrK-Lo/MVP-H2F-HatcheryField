...
#Plotting slopes - T1 length
```{r}
t1_len_df <- rbind(broom.mixed::tidy(len_t1_lew_scaled), broom.mixed::tidy(len_t1_yrk_scaled))
t1_len_df$field_site <- c(rep(c("Lewisetta"), 5), rep(c("York"), 6))

colnames(t1_len_df) <- c("effect", "group", "explanatory", "slope", "se", "Fstat", "df", "pval", "field_site")
#remove rows with Intercept slopes
t1_len_df <- t1_len_df %>% 
  filter(explanatory != "(Intercept)") %>% 
  filter(effect != "ran_pars")

t1_len_plot <- t1_len_df %>% 
  ggplot(aes(x = slope, y = explanatory, color = field_site))+
  geom_point(position = position_dodge2v(height = .5, reverse = T))+
  geom_errorbarh(aes(xmax = slope + 2*se, xmin = slope - 2*se), height = 0, position = position_dodgev(height = -.5))+
scale_color_manual(name = "Field Site", na.translate = F, values = c("Lewisetta" = "#F06292", "York" = "#880E4F"), labels = c("Lewisetta" = "Lewisetta", "York" = "York River"))+
  #ggtitle(c("Effects of EOO and Parent Genetics on Offspring Survival"))+
  labs(subtitle = "Field T1 (May 2023)")+
  xlab("Scaled Slope")+
  ylab("Variable")+
  geom_vline(xintercept = 0, linetype = "dashed")+
  scale_y_discrete(name = "Explanatory Variable", 
...
#Plotting slopes - T2 length
```{r}
t2_len_df <- rbind(broom.mixed::tidy(len_t2_lew_scaled), broom.mixed::tidy(len_t2_yrk_scaled))
t2_len_df$field_site <- c(rep(c("Lewisetta"), 6), rep(c("York"), 6))

colnames(t2_len_df) <- c("effect", "group", "explanatory", "slope", "se", "Fstat", "df", "pval", "field_site")
#remove rows with Intercept slopes
t2_len_df <- t2_len_df %>% 
  filter(explanatory != "(Intercept)") %>% 
  filter(effect != "ran_pars")

t2_len_plot <- t2_len_df %>% 
  ggplot(aes(x = slope, y = explanatory, color = field_site))+
  geom_point(position = position_dodge2v(height = .5, reverse = T))+
  geom_errorbarh(aes(xmax = slope + 2*se, xmin = slope - 2*se), height = 0, position = position_dodgev(height = -.5))+
scale_color_manual(name = "Field Site", na.translate = F, values = c("Lewisetta" = "#F06292", "York" = "#880E4F"), labels = c("Lewisetta" = "Lewisetta", "York" = "York River"))+
  #ggtitle(c("Effects of EOO and Parent Genetics on Offspring Survival"))+
  labs(subtitle = "Field T2 (Nov. 2023)")+
  xlab("Scaled Slope")+
  ylab("Variable")+
  geom_vline(xintercept = 0, linetype = "dashed")+
  scale_y_discrete(name = "Explanatory Variable", 
...
#Plotting slopes - T3 length
```{r}

t3_len_df <- rbind(broom.mixed::tidy(len_t3_lew_scaled), broom.mixed::tidy(len_t3_yrk_scaled))
t3_len_df$field_site <- c(rep(c("Lewisetta"), 7), rep(c("York"), 7))

colnames(t3_len_df) <- c("effect", "group", "explanatory", "slope", "se", "Fstat", "df", "pval", "field_site")
#remove rows with Intercept slopes
t3_len_df <- t3_len_df %>% 
  filter(explanatory != "(Intercept)") %>% 
  filter(effect != "ran_pars")

t3_len_plot <- t3_len_df %>% 
  ggplot(aes(x = slope, y = explanatory, color = field_site))+
  geom_point(position = position_dodge2v(height = .5, reverse = T))+
  geom_errorbarh(aes(xmax = slope + 2*se, xmin = slope - 2*se), height = 0, position = position_dodgev(height = -.5))+
scale_color_manual(name = "Field Site", na.translate = F, values = c("Lewisetta" = "#F06292", "York" = "#880E4F"), labels = c("Lewisetta" = "Lewisetta", "York" = "York River"))+
  #ggtitle(c("Effects of EOO and Parent Genetics on Offspring Survival"))+
  labs(subtitle = "Field T3 (May 2024)")+
  xlab("Scaled Slope")+
  ylab("Variable")+
  geom_vline(xintercept = 0, linetype = "dashed")+
  scale_y_discrete(name = "Explanatory Variable", 
