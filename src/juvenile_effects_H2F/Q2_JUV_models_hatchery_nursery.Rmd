...
260610: KEB: these models are run with the juvenile heterozygosity and allelic richness data which is calculated correctly.
...
#this file contains data on mean heterozygosity and allelic richness in the parent populations - this needs to be updated to be calculated correctly
juv_genetics <- read.csv("results/parent_effects_H2F/juvenile_genetic_effects.csv")
...
#rename column names 
colnames(juv_genetics) <- c("site_name", "ho", "avg_expected_hs", "ar")
```
...
envr_gen <- left_join(quant_eoo, juv_genetics, by = "site_name")
...
# for supplemental material, rename variables for a cleaner graph 
...
The next chunk includes multiple regressions testing the effects of several environmental and genetic parameters on length and survival in the hatchery/nursery. The explanatory variables are 1) salinity quantile 10, 2) salinity quantile 90, 3) temperature quantile 10, 4) mean observed heterozygosity in the juvenile oysters from each site (Ho), and 5) mean allelic richness (Ar) in the juvenile oysters from each site.

## Multiple regression and model selection
#CONSIDER USING JUV HO/AR AT INDIVIDUAL LEVEL

###Day 21 length
```{r}
len21_lm <- lm(length ~ temp10 + sal10+ sal90 + ho + ar, data = len21_envr_gen)
len21_resid <- len21_lm$residuals
shapiro.test(len21_resid) #p = 4.556e-07, data are not normal, 
#plot(len21_lm) #no outliers, homoscedasticity looks good
summary(len21_lm) #everything is significant. Model stats: Adj. R squared = 0.2239, F = 36.66, p < < 2.2e-16

#there's one exceptionally low length from the TX group that is throwing off normality, I'm going to exclude it and retry the models
...
shapiro.test(len21_resid2) #p = 6.083e-05, data are not normal, but improved
#plot(len21_lm2) #no outliers, homoscedasticity looks good
summary(len21_lm2) #everything is significant. Model stats: Adj. R squared = 0.2152, F = 34.85 on 5 and 612 DF, p < 2.2e-16

#model selection
drop1(len21_lm2) #this is the model with the lowest AIC (-5115.3)
