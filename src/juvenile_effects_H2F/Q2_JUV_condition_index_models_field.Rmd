...
This file is for modeling the effects of heterozygosity, allelic richness, and environment-of-origin environmental conditions on oyster condition index during the MVP field experiment. Multicollinearity and VIF assessments for these explanatory variables are conducted in Q2_envr_dist_models_field.Rmd and Q2_envr_quantile_models_field.Rmd. 

CI data and calculations are from ZS. Modeling is based on code from CR.
...
packages <- c("tidyverse", "here", "ggplot2", "lme4", "lmerTest", "car", "broom.mixed", "ggstance", "stringr")
...
ci_df$population <- factor(ci_df$population, levels = c("W1-TX", "W2-LA", "W3-FL", "W4-VA", "S1-LOLA", "S2-DEBY", "W5-NH", "W6-ME"))
...
### Combine CI, envr quantile, and genetic diversity data
```{r}
...
### Combine CI, envr distance, and genetic diversity data
```{r}
...
## Run analyses - Model 1 (Heterozygosity/allelic richness and environmental quantiles)
### t1, CI + quantiles, Lewisetta
```{r t1_ci_lew}
...
```{r}
# save everything
sink(paste0("results/parent_effects_H2F/models/condition_model1_quantiles_results_",Sys.Date(),".txt"))
...
## Run Analyses- Model 2 (Heterozygosity/allelic richness and environmental distance)
### t1, CI + distance, Lewisetta
```{r}
...
```{r}
#save results
sink(paste0("results/parent_effects_H2F/models/condition_model2_envrdist_results_",Sys.Date(),".txt"))
...
#Figure product - York CI model 1 (environmental quantiles)
```{r}
...
#Figure product - Lewisetta CI model 2 (environmental distance)
```{r}
...
ggsave("FigS7B_lew_ci_envrdist.pdf",
       plot = lew_ci2_plot,
       device = "pdf",
       path = "figures/parent_effects_H2F",
       height = 4,
...
#Figure product - York CI model 2 (environmental distance)
```{r}
...
ggsave("FigS7D_york_ci_envrdist.pdf",
       plot = yrk_ci2_plot,
       device = "pdf",
       path = "figures/parent_effects_H2F",
       height = 4,
