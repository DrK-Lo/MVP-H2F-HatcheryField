...
knitr::opts_knit$set(root.dir = "~/MVP_Oyster_Analysis_Camille/MVP-H2F-HatcheryField")
...
library(multcompView) # for tukey
...
# plot
# site color scheme
site_colors <- data.frame(bag_site = c("Lewisetta","YorkRiver"), site_name = c("Lewisetta","York River"), col = c("#6fffd3","#3a9dff"))
...
surv_t3_plot_site <- ggplot(surv_final_time_rename_mg, aes(x = pop, y = surv, fill = bag_site)) + 
  ...
  labs(title = "Final Oyster Survival", x = "Population", y = "Survival") +
  geom_text(aes(label = letters, y = 1), vjust = -0.5, position=position_dodge(1)) +
  scale_fill_manual(name = "Site", label = site_colors$site_name, values = site_colors$col)+
  theme_classic()
surv_t3_plot_site
...
len_t3_plot_site <- ggplot(lengths_final_time_rename_mg, aes(x = pop, y = length, fill = bag_site)) + 
  geom_boxplot(width = 0.5, position=position_dodge(0.5)) +
  labs(title = "Final Oyster Sizes", x = "Population", y = "Length (cm)") +
  scale_fill_manual(name = "Site", label = site_colors$site_name, values = site_colors$col) +
  theme_classic()
  geom_text(aes(label = letters, y = 115), vjust = -0.5, position=position_dodge(0.5)) +
  theme_classic()
len_t3_plot_site
...
# environmental data
env_parents <- read.csv("data/envr_of_origin/envr_summary_stats.csv")[,-1]
colnames(env_parents) <- c("pop","max_temp","min_temp","max_sal","min_sal")
