---
title: "Q1_nursery_performance"
author: "Nicole Mongillo"
output: pdf_document  
date: "2024-02-22"
---

This script is for modeling differences in shell length during the nursery stage. I intend to use linear (mixed) models to examine how survival and length vary by group.
...
packages <- c("tidyverse", "multcompView", "car")
...
We use day 78 as the end of the nursery stage here, as this was what was recorded in the original dataset. This was likely actually day 77 according to SK -KEB. 
...
### Data product: nursery_length.csv
```{r}
#prep data for ANOVA
...
write.csv(no_small_df, "data/performance_H2F/nursery_length.csv")
```
...
### End of hatchery shell length ANOVA

```{r}

# subset for days 15-21
earlystage <- subset(no_small_df, day_an == "15-21")

# ANOVA
earlystage$site_name <- gsub("-", "_", earlystage$site_name)
earlystage.lm <- lm(shell_length_mm ~ site_name, data = earlystage)
shapiro.test(earlystage.lm$residuals)

earlystage.anova3 <- Anova(lm(shell_length_mm ~ site_name, data = earlystage), type = 3)
earlystage.anova3

# Tukey post-hoc
earlystage.anova <- aov(lm(shell_length_mm ~ site_name, data = earlystage))
summary(earlystage.anova)
earlystage_pval <- summary(earlystage.anova)[[1]][["Pr(>F)"]][1]

earlystage.tuk <- TukeyHSD(x = earlystage.anova, conf.level = 0.95)
earlystage_cld <- multcompLetters4(earlystage.anova, earlystage.tuk)
earlystage_dt <- group_by(earlystage, site_name) %>%
  summarise(m = mean(shell_length_mm)) %>%
  arrange(desc(m))
earlystage_cld <- as.data.frame.list(earlystage_cld$site_name)
earlystage_dt$earlystage_cld <- earlystage_cld$Letters
...
### Run ANOVA for end of nursery (day 78) shell lengths
```{r}
...
### Figure product: hatchery length boxplot (day 15-21)
```{r}
earlystage <- earlystage %>% 
  mutate(site_name = site_name %>% 
           fct_relevel("W1_TX", "W2_LA", "W3_FL", "W4_VA", "S1_LOLA", "S2_DEBY", "W5_NH", "W6_ME", "H1_HYBRIDMIX"))

earlystage$shell_length_mm <- as.numeric(earlystage$shell_length_mm)

# plot
earlystage_box <- ggplot(data = earlystage, aes(site_name, shell_length_mm)) +
  geom_boxplot(
    color = c("black"),
    fill = c("#3e4989", "#31688e", "#26828e", "#1f9e89", "#440154", "#482878", "#35b779", "#6ece58", "#fde725"),
    linewidth = .2) +
  scale_x_discrete(
    name = "Treatment group",
    limits = c("W1_TX", "W2_LA", "W3_FL", "W4_VA", "S1_LOLA", "S2_DEBY", "W5_NH", "W6_ME", "H1_HYBRIDMIX"),
    labels = c("W1-TX", "W2-LA", "W3-FL", "W4-VA", "S1-LOLA", "S2-DEBY", "W5-NH", "W6-ME", "H1-HYBRIDMIX")
  ) +
  #  stat_summary(
  #   fun.data = mean_se, fun.args = list(mult = 2), geom = "errorbar", width = 0.2, linewidth = .3,
  #   color = c("white", "white", "white", "white", "white", "white", "black", "black", "black")
  # ) +
  # stat_summary(
  #   fun = mean, geom = "errorbar", aes(ymax = ..y.., ymin = ..y..),
  #   width = 0.75, linewidth = .3, linetype = "solid",
  #   color = c("white", "white", "white", "white", "white", "white", "black", "black", "black")
  # ) +
  ylab("Shell length (mm)") +
  geom_text(data = earlystage_dt, size = 3, aes(label = earlystage_cld, y = Inf, x = site_name), vjust = 1.5) +
  theme_bw() +
  theme(
...
ggsave("FigS3_hatchery_length.pdf",
       plot = earlystage_box,
       device = "pdf",
...
predep_box <- ggplot(data = predep, aes(site_name, shell_length_mm))+
  geom_boxplot(color = c("black"), fill = c("#3e4989", "#31688e", "#26828e", "#1f9e89","#440154","#482878",  "#35b779", "#6ece58", "#fde725"), linewidth = .2)+
    scale_x_discrete(
    name = "Treatment group",
    limits = c("W1_TX", "W2_LA", "W3_FL", "W4_VA", "S1_LOLA", "S2_DEBY", "W5_NH", "W6_ME", "H1_HYBRIDMIX"),
    labels = c("W1-TX", "W2-LA", "W3-FL", "W4-VA", "S1-LOLA", "S2-DEBY", "W5-NH", "W6-ME", "H1-HYBRIDMIX")
  ...
  # ) +
  ylab("Shell length (mm)")+
  geom_text(data = predep_dt, size = 3, aes(label = predep_cld, y = Inf, x = site_name), vjust = 1.5)+
  theme_bw()+
...
ggsave("FigS3_nursery_length.pdf",
       plot = predep_box,
       device = "pdf",
       path = "figures/performance_H2F",
       height = 4,
