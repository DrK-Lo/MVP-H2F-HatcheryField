...
expsites <- read.csv("data/performance_H2F/MVP23-FieldBags - sites.csv")
...
popsdf <- data.frame(pop = c("TX", "LA", "FL", "JR", "DEBY", "LOLA", "NH", "ME", "LARMIX", "SEEDMIX"), order = 1:10, label = c("W1-TX", "W2-LA", "W3-FL", "S1-LOLA", "S2-DEBY", "W4-VA", "W5-NH", "W6-ME", "H1-LARMIX", "H2-SEEDMIX"), cols = c("#3e4989", "#31688e", "#26828e", "#440154", "#482878", "#1f9e89", "#35b779", "#6ece58", "#fde725", "#b5de2b"), shape = c(21, 21, 21, 23, 23, 21, 21, 21, 25 ,25))
mort_df_full <- merge(mort_df, popsdf, by = c("pop"))

# york river
york_surv <- ggplot(data = mort_df_full[mort_df_full$bag_site == "YorkRiver" & !(mort_df_full$pop %in% c("LARMIX", "SEEDMIX")),]) + 
...
# lewisetta
lew_surv <- ggplot(data = mort_df_full[mort_df_full$bag_site == "Lewisetta" & !(mort_df_full$pop %in% c("LARMIX", "SEEDMIX")),]) + 
  geom_line(aes(x = event, y = mean_surv, color = fct_reorder(pop,order))) + 
  scale_color_manual(name = "Population", label = popsdf$label, values = popsdf$cols)+
  geom_point(aes(x = event, y = mean_surv, fill = fct_reorder(pop,order), 
                 shape = fct_reorder(pop,order)),
