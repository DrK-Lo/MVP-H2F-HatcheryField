...
```{r}
# read in data
surv_long <- read.csv("results_field/mortality_long_bags_022025.csv")[,-1]
...
surv_GE1.lew.0 <- lm(surv ~ ho + ar + Salinity_st_dev + Temperature_st_dev, data = surv_mod_gen_env[surv_mod_gen_env$t == 1 & surv_mod_gen_env$bag_site == "Lewisetta",]) # way better vif here
...
drop1(surv_GE1.lew) # get rid of temp_quantile_90
drop1(surv_GE1.lew.0) # get rid of Salinity_st_dev

surv_GE1.0.lew <- lm(surv ~ ho + ar + salinity_quantile_10 + salinity_quantile_90 + temp_quantile_10, data = surv_mod_gen_env[surv_mod_gen_env$t == 1 & surv_mod_gen_env$bag_site == "Lewisetta",])
#plot(surv_GE1.0.lew)
summary(surv_GE1.0.lew) # none signif
drop1(surv_GE1.0.lew) # get rid of temp quantile 10

surv_GE1.1.lew <- lm(surv ~ ho + ar + salinity_quantile_10 + salinity_quantile_90, data = surv_mod_gen_env[surv_mod_gen_env$t == 1 & surv_mod_gen_env$bag_site == "Lewisetta",])
#plot(surv_GE1.1.lew)
summary(surv_GE1.1.lew) # ho signif
drop1(surv_GE1.1.lew) # get rid of sal quantile 90
...
#plot(surv_GE2.0.lew)
summary(surv_GE2.0.lew) # intercept, ho signif
...
drop1(surv_GE3.lew) # get rid of ar

surv_GE3.0.lew <- lm(surv ~ ho + salinity_quantile_10 + salinity_quantile_90 + temp_quantile_10 + temp_quantile_90, data = surv_mod_gen_env[surv_mod_gen_env$t == 3 & surv_mod_gen_env$bag_site == "Lewisetta",])
#plot(surv_GE3.0.lew)
summary(surv_GE3.0.lew) # all except temp_quantile_10 signif
drop1(surv_GE3.0.lew) # get rid of temp_quantile_10
