...
write.csv(tagsbags6, "../../data/performance_H2F/lengths_long.csv")
```
...
# save it
write.csv(lengths_bags_df, "../../data/performance_H2F/lengths_bags.csv")
...
#sink("../../data/performance_H2F/len_sites_02202025.txt")
york1
york2
york3
lew1
...
all_len <- ggplot(data = lengths_mg[lengths$site == "Overall",]) + 
  geom_line(aes(x = monitoring_event, y = length, color = fct_reorder(pop,order))) + 
  scale_color_manual(name = "Population", label = popsdf$label, values = popsdf$cols)+
  geom_point(aes(x = monitoring_event, y = length, fill = fct_reorder(pop,order), 
                 shape = fct_reorder(pop,order)),
